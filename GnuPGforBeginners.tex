...
\begin{slide}[method=direct,toc=]{Generating a new keypair}
  ...
  \item Some people prefer to set an expiry date (say, two years). You
    ...
    use. % This doesn't add any extra security, since anyone who manages to obtain your private key can always extend the expiration date themselves.
  \end{itemize}
\begin{verbatim}
...
\begin{slide}[method=direct,toc=]{Generating a new keypair}
  ...
  \item Avoid using the ``Comment'' field without good reason. % Because other people who sign your keys will see this comment field, and may be hesitant to sign if it contains something they can't verify for themselves.
  \end{itemize}
\vsp
\begin{verbatim}
